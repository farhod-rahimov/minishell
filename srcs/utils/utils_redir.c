...
static	void		work_if_head_null(t_redirect **redir_head, \
		t_args **current_t_arg, char *type)
{
...
static	void		add_file_name(t_redirect *tmp, t_args **current_t_arg, \
		char *file_name)
{
...
void				ft_push_back_redir_list(t_args **current_t_arg, \
		t_redirect *redir_head, char *type, char *file_name)
{
	...
	if (redir_head == NULL)
		return (work_if_head_null(&redir_head, current_t_arg, type));
	tmp = get_last_env_list(redir_head);
	if (tmp->type != NULL && tmp->file_name == NULL)
		return (add_file_name(tmp, current_t_arg, file_name));
